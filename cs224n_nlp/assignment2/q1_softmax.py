...
def softmax(x):
    ...
    ### YOUR CODE HERE
    reshape_size = (x.get_shape()[0].value, 1)
    column_max = tf.reshape(tf.reduce_max(x, axis=1), reshape_size)
    out = tf.exp(x - column_max) / tf.reshape(tf.reduce_sum(tf.exp(x - column_max), axis=1), reshape_size)
    ### END YOUR CODE
